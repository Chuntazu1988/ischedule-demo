...
def has_break_gap_in_schedule(assignments, emp_name, emp, new_task_start, emp_tasks=None):
    ...
    # "רק שומר TSA שמסתיים מוקדם [ב]מוגבלות" — found via real data: שגיב
    # נגווקר, guard duty ending 05:00, immediately stacked onto LY279 at
...
def is_available(assignments, emp_name, start, end, emp_row=None, role=None, flight_gate=None, emp_tasks=None):
    ...
    _ss_av_m = None
    if emp_row is not None:
        from utils.helpers import is_time_text, time_to_minutes, clean_text as _ct
        _cand_starts = []
        _ss_av_str = _ct(emp_row.get("תחילת משמרת", ""))
        if is_time_text(_ss_av_str):
            _cand_starts.append(time_to_minutes(_ss_av_str))
        for _avw in _ct(emp_row.get("זמינות", "")).split(","):
            _avw = _avw.strip()
            if "-" in _avw:
...
def sort_candidates(candidates, assignments, role, task_start=None, task_end=None,
    ...
    # ── שומר TSA priority ────────────────────────────────────────────────────
    # Fill order (user rule 2026-07-15): 0=פורשים/ילד עובדים (always first
    # priority) 1=דייל בטרייני (attendant-in-training — ONLY when no restricted
    # worker is free; this is the one situation where a trainee may be
    # separated from their paired mentor) 2=regular non-runner דיילים.
    # Two bugs fixed here: (1) the tiers were in the wrong order — trainees
    # were sorted AHEAD of restricted workers, when restricted workers must
    # always come first; (2) "trainee" was checked via "טרייני רצ" (TL/ר"צ
    # trainee), an unrelated column — the correct flag for an attendant
    # trainee is TRAINEE_ATTENDANT_COL ("דייל בטرייני").
    if role == "שומר TSA":
        _is_restricted  = (_yn("ילד עובדים") + _yn("פורשים")) > 0
        _is_att_trainee = _yn(TRAINEE_ATTENDANT_COL) > 0
        tsa_guard_priority = np.where(
            _is_restricted, 0,
            np.where(_is_att_trainee, 1, 2)
        ).astype(np.float32)
    else:
    ...
    #     real data — TL#37 holds the certification but only TL#43/בר
    #     שיילו are under a "פיקוח TSA" header for this shift).
    ...
    # (03:30-12:30, already had 3 tasks) lost LY5467 (10:50-11:50) to לוצקי
    # חנה (08:30-16:00, only 1 task) purely on tl_cnt, even though TL#26
    ...
    if role == "ראש צוות":
        ...
        # other specialists — found via real data 2026-07-15: אונציאיה
        # אנסטסיה (TL+TSA dual-certified) went from 6 ר"צ tasks to 0 after an
        ...
        if _is_early_morning_flight:
            ...
            keys = (task_count, nearby, prox, first_task_elapsed, tl_ready, peak_tl_pref, shift_tail, recent_task, tl_cnt, night_flight_done, shift_pri, first_task_buffer, transfer_lr, upcoming_lr, shift_bad)
        else:
            ...
            keys = (task_count, nearby, prox, first_task_elapsed, tl_ready, peak_tl_pref, shift_pri, shift_tail, recent_task, tl_cnt, shift_urgency, first_task_buffer, transfer_lr, upcoming_lr, shift_bad)
        ...
        # need never materializes. Found via real data 2026-07-15: בן דוד
        # נועה (02:00-11:00, ר"צ-qualified, last ר"צ task ends 09:05 — under
        ...
        keys = (task_count, nearby, prox, first_task_elapsed, att_priority, shift_pri, shift_tail, idle_first, night_lateness_fit, recent_task, group_balance, em_split, dual_qual, tsa_preserve, peak_em_tl, tl_att_pref, tl_preserve, first_task_buffer, transfer_lr, upcoming_lr, shift_bad)
    elif role == "מפקח TSA":
        ...
        keys = (same_pier_pref, inspector_shift, task_count, nearby, prox, first_task_elapsed, shift_pri, shift_tail, recent_task, dual_qual, first_task_buffer, transfer_lr, upcoming_lr, shift_bad)
    elif role == "שומר TSA":
        keys = (same_pier_pref, task_count, nearby, prox, first_task_elapsed, shift_pri, recent_task, tsa_guard_priority, first_task_buffer, transfer_lr, upcoming_lr, shift_bad)
    else:
        keys = (task_count, nearby, prox, first_task_elapsed, shift_pri, shift_tail, night_lateness_fit, recent_task, dual_qual, tsa_preserve, first_task_buffer, transfer_lr, upcoming_lr, shift_bad)
    ...
    for _, flight in flights_df.iterrows():
        ...
        for role in ROLE_ORDER:
            ...
            for i in range(amount):
                ...
                if role == "ראש צוות":
                    # מנהל כר"צ eligible without restriction
    ...
    for _gf_name, _gf_all_tasks in list(_by_name.items()):
        ...
        if _gf_primary == "ראש צוות":
            ...
            if clean_text(str(_gf_row.get("מתדרכת", ""))) == "כן":
                ...
                _gf_roles_try = ["ראש צוות", "מתאם תורים", "דייל"]
            ...
            for (_gs, _ge, _is_tail) in _gaps:
                ...
                for _fill_role in _gf_roles_try:
                    ...
                    for (_fs, _fe, _fk3, _req_n, _gate3, _fls3, _fle3, _fl_row3, _body3) in _fl_list_r:
                        ...
                        # לדלפקים ולשבץ עובד אחר" — found via real data: ברנע
                        # מיכל, 02:00-11:00, LY237/LY347 preempted by TL workers,
...
def upgrade_teamleads(assignments_df, employees_df):
    ...
    def _spans_overlap(s1, e1, s2, e2):
        a1, b1 = _span_minutes(s1, e1)
        a2, b2 = _span_minutes(s2, e2)
...
def optimize_tl_continuity(assignments_df, employees_df):
    ...
    for a_name, a_tasks in list(by_worker.items()):
        ...
        if a_row is None or len(a_tasks) < 2:
            continue
...
def reserve_dual_certified_for_tsa(assignments_df, employees_df):
    ...
    def _ov(s1, e1, s2, e2):
        """Overlap with noon pivot (night hours belong to the ops-day tail)."""
        def _n(t):
...
def get_qualified_candidates_for_swap(schedule_df, employees_df, flight_num, role_base, task_idx):
    ...
    for _, emp_row in certified.iterrows():
        ...
        if not is_within_shift(emp_row, task_start, task_end, task_terminal=_swap_term):
            continue
        conflict = False
        for (_ps, _pe) in _tasks_by_name.get(name, []):
            try:
...
def force_assign_worker(schedule_df, slot_idx, worker, employees_df, keep_prior=True,
    ...
    * keep_prior=False — "שבץ במקום" / move in place (user rule 2026-07-22).
    ...
    if _s is not None and _e is not None:
        ...
        for _idx in df.index[df["עובד"].astype(str).str.strip() == str(worker).strip()].tolist():
            if _idx == slot_idx:
                continue
...
def protect_early_shift_preflight_breaks(schedule_df, employees_df):
    ...
    # itself too short (user rule 2026-08-05, found via real data: נועה
    # פליק, 02:00-09:30, every gap under 45 min including the 25-min tail —
...
def pair_trainee_attendants(schedule_df, employees_df):
    ...
    for _, _r in employees_df.iterrows():
        ...
        if not _mentor:
            continue
        _trainees.append((clean_text(_r.get("שם", "")), _wordset(_r.get("שם", "")), _wordset(_mentor)))
    if not _trainees:
        return df
    # mentor word-set -> the trainees attached to them (a mentor may have more
    # than one). Used by step 2c below.
    _trainees_by_mentor = {}
    for _n, _ts, _ms in _trainees:
    ...
    for _tr_name, _tr_set, _mentor_set in _trainees:
        ...
        # every דייל row entirely (found via real data 2026-07-22 — אדר
        # טרנטייב never got seated because her mentor's ר"צ row starts 10 min
        ...
        for _fn in _mentor_flights:
            ...
            #    ADDING the trainee (a restricted worker → +1).
            _open = [_i for _i in _dil if _i in _open_dil]
            ...
            if _disp is None:
                ...
                #    דייל 1, agent#47 in דייל 2, leaving her own trainee שמחה
                #    דסטה סיום with no seat).
            ...
            # to_datetime_time first (found via real data 2026-07-31: מעיין
            # דיין, shift 21:00-07:00, re-seated onto LY007 09:10-10:15 — over
...
def balance_workload(schedule_df, employees_df, terminal="1"):
    ...
    for _giver in _loaded:
        ...
        # worker who already sits in one of the terminal's dead zones (מיכל
        # מרידן's day carries a structural 125-min gap) would otherwise fail
    ...
    for _tset, _mset in _mentor_of.items():
        ...
        for _flt in sorted(_mentor_flights.get(_mset, ())):
            ...
            if not any(_ws(_c) == _tset for _c in _cands2):
                ...
                # LY081: "צריך לתת עדיפות לטרייני של ר\"צ TL#39 על פני ניר
                # סטמלר, כי TL#39 היא חובה בטיסה הזאת"). Accept a SHORT
...
def _ar_can_fill(emp_row, role):
    ...
    if role == "מפקח TSA":
        ...
        # rule as the primary build pass; user-reported 2026-07-29).
        if str(emp_row.get("מחלקה מקורית", "")).strip():
            return False
...
def _ar_conflicts(sched, name, start_m, end_m, skip_idx=None):
    ...
    if uncovered_indices:
        ...
        for name, emp in unscheduled_workers:
            ...
            for task_idx in uncovered_indices:
                ...
                if not _ar_can_fill(emp, role):
                    continue
                if _ar_conflicts(sched, name, start_m, end_m):
                    continue
                if not _ar_in_shift(emp, start_m, end_m, task_terminal=get_terminal(clean_text(str(task.get("_gate", ""))))):
                    continue
                coverage += 1
