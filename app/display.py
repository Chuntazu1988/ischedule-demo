...
def build_next_task_labels(result_df, employees_df, built_until_minutes=None):
    ...
    for _tne, _ttasks in _emp_sorted.items():
        ...
        if "1" in _sh_tags:
            ...
            for _wi, _w in enumerate(_wins):
                if "-" not in _w or _wi >= len(_sh_tags) or _sh_tags[_wi] != "1":
                    continue
                _ws, _we = _w.split("-", 1)
    ...
    for idx, row in timed_df.sort_values("_loop_m", kind="stable").iterrows():
        ...
        if stage == 0:
            ...
            if _fp_first and _fp_first[0][0] == idx:     # this is the worker's first task
                ...
                if _fp_s and is_time_text(_fp_s):
                    ...
                    # found via real data 2026-07-15: agent#51, שירי ביירון
                    # שוורצמן, trainee-agent#3, agent#53, agent#44, agent#28
                    # (08:00-17:00) and סימן טוב דנה, trainee-agent#9, agent#12,
                    # agent#6, agent#36, agent#48 (09:30-17:30) and עופרי
                    # לוי (11:00-19:00) all had a real but LATE between-task gap
                    ...
                    # with the SAME between-flights problem on ר"צ-role tasks (פלח
                    # ליאן, and TL#7 herself with a different real day's task
                    ...
                    if _existing_slot_idx is None or _existing_too_late:
                        ...
                        # start really gives only 30 usable minutes, e.g. גל
                        # ישראל 03:25-04:10 before an 04:10 flight).
                        _fp_walk = 15
                        if (_fp_s, _fp_e) == ("03:00", "05:00"):
                        ...
                        if task_start_m_now >= _fp_earliest_m + _fp_rb + _fp_walk:
                            ...
                            if _fp_room_to_spare:
                                ...
                                # טיסה" — agent#53, agent#51, agent#44 / agent#52, נוב
                                # שני, סימן טוב דנה, trainee-agent#9, agent#37, עידן
                                # לבינסקי, agent#6, agent#48 / agent#49).
                                _fp_b_start = _fp_s_m
                                ...
                                # right before it — agent#20, לוי לילי לוטם, מסלטון
                                # ליבי, agent#26).
        ...
        if action in ("break", "הפסקה", "refresh", "רענון") and not is_last_task:
            ...
            if gap_to_next is not None and gap_to_next < _req_break_mins:
                action = None
...
def to_departures_report_excel_bytes(flights_df, schedule_df, employees_df, terminal_view="both"):
    ...
    _SHOW_LABEL_ROLES_EXACT = {"מפקח TSA", "שומר TSA", "מתאם תורים", "טרייני רצ", 'טרייני ר"צ'}
    ...
    for _, flight in flights_df.iterrows():
        ...
        for _, task in tasks.iterrows():
            ...
            _name_part = f"{worker} ({rich_role})" if _show_label else worker
        ...
        for i in range(max_rows):
            values = [
                ...
                (f"{dep_time} ({boarding_time})" if boarding_time else dep_time) if i == 0 else "",
