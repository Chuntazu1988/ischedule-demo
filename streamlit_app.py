...
with st.sidebar:
    ...
    if _loaded_daily or _loaded_emp or _loaded_t1:
        st.markdown(
            '<div style="direction:rtl;font-size:11px;color:#888;margin-bottom:4px;">קבצים טעונים:</div>',
            unsafe_allow_html=True,
        )
        for _fname in filter(None, [_loaded_daily, _loaded_t1, _loaded_emp]):
            st.markdown(
...
if not daily_file or not employees_file or not _has_fids:
    import re as _re
...
def _upload_fingerprint():
    ...
    _parts = []
    for _tag, _f in (("d", daily_file), ("e", employees_file), ("t", st.session_state.get("t1_file_obj"))):
        if _f is None:
    ...
    def _terminal_label_for(row, shift=None):
        ...
        if "@" in _tr:
            ...
            if not (shift is not None and _two_shifts and _to not in {t for _, _w, t in _avail_windows_sorted(row, shift)}):
                # הערה ללא שעה ("משה לוי -טרמינל 1") = כל המשמרת בטרמינל היעד,
    ...
    if active_main_tab == TAB_WORKFLOW:
        ...
        if _wf_labeled_outer.empty:
            ...
            def _render_wf_tab():
                ...
                # while the schedule may display it reversed ("בן דוד נועה"),
                ...
                for _row_id in sorted(_wf_groups.keys(), key=_emp_shift_key):
                    ...
                    # not consistently — found via real data: agent#21, חזות
                    # דניאל showed no break chip at all despite having the same
                    ...
                    if tasks and _transfer_raw:
                        ...
                        _pre_break_chip = (
                            ...
                            f'{_t_b_start_m//60:02d}:{_t_b_start_m%60:02d}–'
                            f'{_t_b_end_m//60:02d}:{_t_b_end_m%60:02d} '
                    ...
                    # (found via real 12.07 data: טולסטיך חנה, TL#25,
                        ...
                        if not _brk_already0:
                            ...
                            if _ft0_m < (3 * 60 + 30 + _rb0 + _walk0):
                                ...
                                if _pb0_start_m >= 2 * 60 + 45:
                                    _pre_break_chip = (
                                        f'<span class="wf-chip wf-break">☕ הפסקה '
                                        f'{_pb0_start_m//60:02d}:{_pb0_start_m%60:02d}–'
                                        f'{_pb0_end_m//60:02d}:{_pb0_end_m%60:02d} '
                    ...
                    if (not _pre_break_chip and tasks and _sh_m >= 0
                        ...
                        # data: בן דוד נועה, 02:00 shift, break correctly
                        ...
                        if _pre_gap_min >= 90 and not _break_already_placed and _no_other_gap:
                            ...
                            _pre_break_chip = (
                                ...
                                f'{_pb_start_m//60:02d}:{_pb_start_m%60:02d}–'
                                f'{_pb_end_m//60:02d}:{_pb_end_m%60:02d} '
                    ...
                    # סימן טוב דנה, trainee-agent#9, agent#12, agent#6, agent#36,
                    ...
                    if tasks and _sh_m >= 0 and not _proactive_break_chip and not _proactive_break_active and not _transfer_raw:
                        ...
                        _proactive_shift_windows = {
                            ...
                            # (user-reported 2026-08-02: עידן לביסקי/agent#5/קארין
                            # אביטן/עדן גוליית יעקב, all 13:00-00:30 with one late flight,
                        ...
                        if _w_s and _wfirst_start_m is not None:
                            ...
                            if (_w_break_label and (not _w_break_shown or _is_0200_short_w)
                                ...
                                if _w_label == "הפסקה יזומה":
                                    ...
                                    _pre_break_chip = (
                                        f'<span class="wf-chip wf-break">☕ הפסקה {_w_disp_s}–{_w_disp_e} '
                                        f'לפני הירידה לטיסות</span>' + SEP
                                    )
                                _suppress_break_mentions = True
                        ...
                        if not _is_last:
                            if _suppress_break_mentions:
                                ...
                                if 0 <= _br_s_m < 1440:
                                    ...
                                    _br_win = (f' {_br_s_m//60:02d}:{_br_s_m%60:02d}–'
                                               f'{_br_e_m//60:02d}:{_br_e_m%60:02d}')
                                ...
                                if _hlp_quiet and 0 <= _hlp_e < 1440 and 0 <= _hlp_s < 1440                                         and ((_hlp_s - _hlp_e) % 1440) > 20:
                                    ...
                                    # (הנחיית משתמשת 2026-08-21, על מעיין
                                    # רוזנברג וTL#14).
                    ...
                    if _emp_cat_tag == "ט":
                        ...
                        # word-order-reversed from their schedule name (e.g. "נועה
                        # בן דוד" vs "בן דוד נועה") was still wrongly reported as
                        # off-shift (found via real data 2026-08-02: שמחה דסטה
